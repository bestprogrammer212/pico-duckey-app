...
function waitForMount(labels, timeoutMs = 60000) {
  return new Promise((resolve, reject) => {
    ...
    const iv = setInterval(() => {
      ...
      else if (Date.now() - start > timeoutMs) { clearInterval(iv); reject(new Error('Timed out waiting for device')) }
    }, 1000)
  })
}
...
function waitForUnmount(mountPath, timeoutMs = 30000) {
  return new Promise((resolve, reject) => {
    ...
    const iv = setInterval(() => {
      ...
      else if (Date.now() - start > timeoutMs) { clearInterval(iv); reject(new Error('Timed out waiting for unmount')) }
    }, 500)
  })
}

module.exports = { findBootsel, findCircuitpy, waitForMount, waitForUnmount, BOOTSEL_LABELS, CIRCUITPY_LABELS }const fs = require('fs')
const os = require('os')
const BOOTSEL_LABELS = ['RPI-RP2', 'RP2350', 'RPI-RP2350', 'RPI-RP2040']
const CIRCUITPY_LABELS = ['CIRCUITPY']
function findMount(labels) {
